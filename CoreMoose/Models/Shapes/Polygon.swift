...
public struct Polygon: Shape {
    ...
    public init(count: Int, relativeCornerRadius: CGFloat) {
        self.count = max(count, 3)
        cornerRadius = .relative(min(max(relativeCornerRadius, 0.0), 1.0))
    }
    ...
    public func path(in rect: CGRect) -> Path {
        ...
        if relativeCornerRadius == 0.0 {
            ...
            for i in 0..<count {
                ...
                let cornerCircle: RounedCornerCircle = rounedCornerCircle(
                    leading: prevPoint,
                    center: currentPoint,
                    trailing: nextPoint,
            ...
            let circleFrame = CGRect(
                origin: CGPoint(
                    x: size.width / 2 - circleSize.width / 2,
    ...
    func maxRadius(size: CGSize) -> CGFloat {
        ...
        let pointDistance = CGPoint(
            x: abs(midPoint.x - centerPoint.x),
    ...
    private func rounedCornerCircle(
        _ p: CGPoint,
        _ p1: CGPoint,
        _ p2: CGPoint,
        _ r: CGFloat)
